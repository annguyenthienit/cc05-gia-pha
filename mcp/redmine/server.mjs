...
//#region package.json
var name = "@onozaty/redmine-mcp-server";
var version = "1.2.0";
...
var author = "onozaty";
var license = "MIT";
var bugs = { "url": "https://github.com/onozaty/redmine-mcp-server/issues" };
var homepage = "https://github.com/onozaty/redmine-mcp-server#readme";
var devDependencies = {
	"@types/node": "^20.0.0",
	"orval": "^7.10.0",
	"tsdown": "^0.12.9",
...
const compilePattern = (value, varName) => {
	if (!value) return null;
	try {
		return new RegExp(value);
	} catch (e) {
		throw new Error(`Invalid regex in ${varName}: "${value}" - ${e.message}`);
	}
};
/**
...
const loadConfig = () => {
	...
	const toolsAllowPattern = compilePattern(process.env.REDMINE_MCP_TOOLS_ALLOW_PATTERN, "REDMINE_MCP_TOOLS_ALLOW_PATTERN");
	const toolsDenyPattern = compilePattern(process.env.REDMINE_MCP_TOOLS_DENY_PATTERN, "REDMINE_MCP_TOOLS_DENY_PATTERN");
	return {
		readOnlyMode,
		redmineUrl,
		redmineApiKey,
...
const allTools = /* @__PURE__ */ new Map();
const isToolEnabled = (toolName, toolType) => {
	if (config.readOnlyMode && toolType === ToolType.WRITE) return false;
	if (config.toolsDenyPattern && config.toolsDenyPattern.test(toolName)) return false;
	if (config.toolsAllowPattern && !config.toolsAllowPattern.test(toolName)) return false;
	return true;
};
/**
...
const registerTool = (toolName, description$1, toolType, schemas, handler) => {
	...
	if (!isToolEnabled(toolName, toolType)) return;
	server.tool(toolName, description$1, schemas, handler);
};
const logServerMode = () => {
	const toolEntries = Array.from(allTools.entries());
	const enabledCount = toolEntries.filter(([name$1, type$1]) => isToolEnabled(name$1, type$1)).length;
	const disabledCount = toolEntries.length - enabledCount;
	console.error("Starting Redmine MCP Server");
	if (config.readOnlyMode) console.error("  - Read-only mode: write operations disabled");
	if (config.toolsAllowPattern) console.error(`  - Allow pattern: ${config.toolsAllowPattern.source}`);
	if (config.toolsDenyPattern) console.error(`  - Deny pattern: ${config.toolsDenyPattern.source}`);
	console.error(`Available tools: ${enabledCount} operations`);
	if (disabledCount > 0) console.error(`Disabled tools: ${disabledCount} operations`);
};
...
registerTool("getProjects", "List projects", ToolType.READ_ONLY, {
...
registerTool("getTimeEntryActivities", "List time entry activities", ToolType.READ_ONLY, { pathParams: getTimeEntryActivitiesParams }, getTimeEntryActivitiesHandler);
